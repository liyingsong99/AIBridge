...
- Keep `Templates~/Rules/AIBridge.RootRule.md` minimal: explicit project-root-relative `$CLI` binding, common commands, quick-task/workflow routing, Skill root hint, package version, and compact capability summary.
...
- `Templates~/Rules/AIBridge.RootRule.md` 只保留明确的项目根目录相对 `$CLI` 绑定、常用命令、快速任务/工作流任务路由、Skill 根目录提示、包版本和 compact 能力摘要。
