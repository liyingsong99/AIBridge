...
Installed AIBridge Skills are written to the project-root `.skills/` directory by default. You can change that shared source directory in the Skills Installation tab, for example to `.skill`. For tools that discover Skills through plugin manifests, AIBridge also generates standard `skills/` mirror directories, such as `plugins/aibridge-skills/skills/` for Codex. Each AI tool only receives a minimal RootRule or plugin adapter. The RootRule only includes the fixed CLI path, common commands, shared Skill root, and `aibridge-development-workflow` entry point; full routing and checklists live in the workflow Skill. Command references are generated under each installed Skill's `references/` directory.
