...
After Code Index is enabled, Unity Editor can generate the snapshot and prewarm the daemon from `AIBridge > Settings > Code Index` after startup idle time. The same panel controls snapshot auto refresh, warmup mode, text fallback, PackageCache source inclusion, ignored assembly/source-path patterns, and quit cleanup. By default, Code Index ignores `Unity.*` assemblies and `Library/PackageCache/com.unity.*` / `Packages/com.unity.*` source paths to reduce Unity package noise while keeping excluded assemblies available as metadata references for project semantic resolution. Daemon startup now uses semantic warmup by default: it loads the snapshot name index and builds the Roslyn workspace in the background, moving the cold-start cost out of the first semantic query. Set warmup mode to `light` when a project prefers lower startup memory/CPU and accepts deferred Roslyn construction. Reference queries also use the snapshot token index to narrow Roslyn candidate files when possible. `status` reports `workspaceMode=unity-snapshot`, snapshot metadata, excluded counts, loaded project/document counts, and `stale=true` when the daemon must reload a newer snapshot.
