...
![Package 1.5.0](https://img.shields.io/badge/Package-1.5.0-5b6cff?style=flat-square)
...
| 快速精确文本搜索 | “找出所有提到 PaymentService 的 Prefab 或配置。” | 本地 `text_index` 对项目文本做 literal/regex 搜索 | 路径、行号、预览结果 |
...
- **本地 Text Index**：`text_index` 会在 `.aibridge/text-index/` 下生成落盘增量索引，用于快速 literal 和 regex 搜索脚本、配置、YAML、Prefab、Scene、文档等项目文本文件。它返回精确文件/行匹配，并标记 `semantic=false`；v1 不包含语义搜索或 embedding。
...
首次 build 会写入默认配置 `.aibridge/text-index/config.json`。默认包含 `Assets/`、`Packages/`、`ProjectSettings/`、`.codex/` 和 `.aibridge/plan/`，跳过 `Library/`、`Temp/`、`obj/`、`bin/`、`.git/`、`.svn/` 等生成目录，并索引 `.cs`、`.json`、`.md`、`.prefab`、`.unity`、`.asset`、`.shader`、`.uss`、`.uxml` 等常见文本扩展名。
...
7. 使用 `text_index` 精确搜索脚本、配置、YAML、Prefab、Scene、文档和非 C# 文件；索引不可用或范围不适用时再降级 `rg`。
