...
安装后的 AIBridge Skills 默认统一写入项目根目录 `.skills/`，也可以在 Skills 安装页签改成 `.skill` 等项目内相对目录。不同 AI 工具只写入各自的最小 RootRule 或插件适配层。RootRule 只包含固定 CLI 路径、常用命令、共享 Skill 根目录和 `aibridge-development-workflow` 入口；完整路由和检查清单由 workflow Skill 维护。命令说明会生成到各 Skill 的 `references/` 目录。
