...
| C# 声明定位 | “帮我找到 PlayerController 类或 OpenInventory 方法在哪个脚本里。” | 只读 `code_index` 声明名检索，用于返回声明文件和声明位置 | 候选文件路径和声明位置 |
...
- `code_index` 默认关闭；需要快速做 C# 声明名定位时，请在 `AIBridge > Settings > Code Index` 中启用。它依赖 Editor 生成的 Unity 编译快照，不依赖 `.sln/.csproj`、Visual Studio、Rider 或 Build Tools；缺少快照时可在 AIBridge 设置中执行 Code Index 预热。
...
安装后的 AIBridge Skills 会写入各已选工具自己的默认 skills 目录，例如 Codex 使用 `.codex/skills/`。也可以在 `Workflows > Skills` 页签设置自定义目录，但自定义目录可能无法被 AI 工具自动发现。不同 AI 工具会收到各自的最小 RootRule；只有自定义目录需要时才写入插件适配层并引用该 Skill 根目录。RootRule 包含固定的项目根目录相对 CLI 路径、明确的 `$CLI` 绑定、常用命令、Skill 根目录和 `aibridge-development-workflow` 入口。多分支路由、针对性检查清单、高级 workflow 编排规则和命令说明都放在已安装 workflow Skill 及其 `references/` 目录中。
...
`compile dotnet` 会用有界读取器持续排空子进程输出。原始输出最多保留 200,000 字符，诊断单行最多 32,768 字符，错误和警告各最多返回 1,000 条；结果仍会提供完整总数、过滤数、省略数和截断状态。
...
嵌套 `workflow run-cli` 的 stdout/stderr 每个流最多保留 1,000,000 字符。命令结果会返回 `stdoutCharsRead`、`stderrCharsRead`、`stdoutTruncated` 和 `stderrTruncated`；一旦发生截断，该步骤直接失败，避免把不完整 JSON 当成有效证据。
...
`code_index` 是 CLI-only 的只读轻量查询入口，默认关闭；只有项目需要快速做 C# 声明名定位时，才在 `AIBridge > Settings > Code Index > Enable Code Index` 中启用。它不要求打开 Rider、VS Code、Cursor、Visual Studio、Build Tools，也不要求 Unity 生成 solution。Unity Editor 会把编译快照写入 `.aibridge/code-index/snapshot/`；当前工程本地的 `AIBridgeCodeIndex` daemon 读取该快照，并直接消费 snapshot name index 做轻量查询。
...
- `derived`
- `diagnostics --all`
- 首轮完整 Roslyn workspace 语义预热 / 全工程语义建表
- 单队列 daemon 上的 `batch` 语义查询
