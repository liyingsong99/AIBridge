...
![Package 1.4.10](https://img.shields.io/badge/Package-1.4.10-5b6cff?style=flat-square)
...
| Player / 手机端调试 | “Android 包里登录按钮没反应，帮我收集证据。” | Runtime discover/status/logs/screenshot/perf、handler 调用 | target id、日志、截图、性能数据 |
...
让 AI 进入 Play Mode，使用 `input` 对 UGUI/EventSystem 界面执行点击、拖拽、长按，再用日志和截图确认结果。它适合验证按钮、背包拖放、运行时面板、弹窗流程和基础交互回归。
