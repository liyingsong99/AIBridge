...
| Code Index | 快速定位 C# 声明文件或声明位置 | `$CLI code_index status`，确认已启用且 snapshot / name index 可用 | 使用宿主自带的搜索/读取工具 |
...
- Code Index disabled/unavailable/not-ready/stale：不要反复重试同一查询；使用宿主自带的搜索/读取工具、相邻文件和常规源码阅读；如需说明，单独写成工具策略，不混入 harness 可用性宣告。
