...
| Code Index | C# 符号、定义、引用、调用者、诊断查询 | `$CLI code_index status`，要求 `semantic=true` | 使用宿主自带的搜索/读取工具 |
...
- Code Index disabled/unavailable/semantic=false：不要重试同一语义查询；使用宿主自带的搜索/读取工具、相邻文件和常规源码阅读；如需说明，单独写成工具策略，不混入 harness 可用性宣告。
