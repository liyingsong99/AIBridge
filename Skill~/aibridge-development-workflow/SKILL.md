...
- `references/branches/requirements.md`：需求讨论模式、方案确认和方案写入规则，仅在需求未明确、边界待定或用户要求先讨论时读取。
...
- `-> ...`：只用于当前 phase/step；下一行可直接写必要说明，Skills、输出目标等继续换行展示。
...
执行中如果步骤变化，输出一个简短步骤块即可：先写 `-> <步骤>`，下一行直接写必要说明。需求讨论、方案说明、最终结果、验证结果、错误或阻塞说明仍按任务需要完整说明。
...
- 如果需求边界、验收标准或方案方向不清晰，先进入 `references/branches/requirements.md` 对应的需求讨论模式，确认后再继续正式分支选择。
...
- **需求讨论模式**：需求边界、验收标准或方案方向不明确，或用户要求先分析/先确认时，先进入需求讨论模式，完成方案确认和必要的方案自动写入到 `.aibridge/plan`，再继续正式分支。

## 风险门控

低风险且目标明确时继续执行；以下情况先按 `references/risk-gates.md` 进入需求讨论模式并等待确认：
